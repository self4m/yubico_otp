-- 创建yubico_otp数据库（如果不存在）
CREATE DATABASE IF NOT EXISTS yubico_otp;

-- 创建MySQL用户
CREATE USER 'yubico_otp'@'localhost' IDENTIFIED BY 'yubico_otp';

-- 授权yubico_otp用户访问数据库
GRANT INSERT, SELECT, UPDATE, DELETE ON yubico_otp.* TO 'yubico_otp'@'localhost';
